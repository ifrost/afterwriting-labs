...
Custom fonts can be passed to CLI via a JS file with PDF fonts. The JS file can be generated by running:

    > aferwriting-fonts --regular MyFontRegular.ttf --italic MyFontItalic.ttf --bold MyFontBold.ttf --bolditalic MyFontBoldItalic.ttf --output my-font.js

Or if you're running the CLI from sources: `node tools/fonts-converter ...`

Only the regular style of the font is required so it's enough to pass:

    > afterwriting-fonts -r ./RegularFont.ttf -o my-font.js
